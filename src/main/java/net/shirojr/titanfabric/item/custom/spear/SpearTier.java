package net.shirojr.titanfabric.item.custom.spear;

import net.minecraft.item.ToolMaterial;
import net.minecraft.util.Formatting;
import net.shirojr.titanfabric.item.custom.material.TitanFabricToolMaterials;
import net.shirojr.titanfabric.util.effects.WeaponEffect;

import java.util.List;

/** Central, authoritative balancing data for every TitanFabric spear. */
public enum SpearTier {
    CITRIN(TitanFabricToolMaterials.CITRIN, 5, 750, 8 * 20, 25, 1.25f, WeaponEffect.POISON, true, Formatting.YELLOW),
    EMBER(TitanFabricToolMaterials.EMBER, 6, 1250, 7 * 20, 30, 1.50f, WeaponEffect.FIRE, true, Formatting.RED),
    DIAMOND(TitanFabricToolMaterials.DIAMOND, 6, 1750, 7 * 20, 30, 1.50f, null, true, Formatting.AQUA),
    LEGEND(TitanFabricToolMaterials.LEGEND, 7, 0, 6 * 20, 35, 1.75f, null, true, Formatting.LIGHT_PURPLE),
    NETHERITE(TitanFabricToolMaterials.NETHERITE, 8, 2250, 5 * 20, 40, 2.00f, null, false, Formatting.DARK_GRAY);

    private final ToolMaterial material;
    private final int damage;
    private final int durability;
    private final int throwCooldown;
    private final int range;
    private final float rangeModifier;
    private final WeaponEffect innateEffect;
    private final boolean weaponEffects;
    private final Formatting nameColor;

    SpearTier(ToolMaterial material, int damage, int durability, int throwCooldown, int range, float rangeModifier, WeaponEffect innateEffect,
              boolean weaponEffects, Formatting nameColor) {
        this.material = material;
        this.damage = damage;
        this.durability = durability;
        this.throwCooldown = throwCooldown;
        this.range = range;
        this.rangeModifier = rangeModifier;
        this.innateEffect = innateEffect;
        this.weaponEffects = weaponEffects;
        this.nameColor = nameColor;
    }

    public ToolMaterial material() { return material; }
    public int damage() { return damage; }
    public int durability() { return durability; }
    public int throwCooldown() { return throwCooldown; }
    public int range() { return range; }
    public float rangeModifier() { return rangeModifier; }
    public WeaponEffect innateEffect() { return innateEffect; }
    public boolean weaponEffects() { return weaponEffects; }
    public Formatting nameColor() { return nameColor; }
    public List<WeaponEffect> supportedEffects() { return weaponEffects ? List.of(WeaponEffect.values()) : List.of(); }
}
