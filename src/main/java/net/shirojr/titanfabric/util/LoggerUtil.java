...
public class LoggerUtil {
    /**
     * Helpful Utility class for clean LOGGER handling
     */
    private LoggerUtil() {
        // private ctor to avoid instantiating this util class
    }
    ...
    public static void devLogger(String input, boolean isError, Exception exception) {
        ...
        if (!isError) {
            TitanFabric.LOGGER.info(printText);
            return;
        }
        if (exception == null) {
            TitanFabric.LOGGER.error(printText);
            return;
        }
        TitanFabric.LOGGER.error(printText, exception);
    }
}
